...
void RDWIN_DrawPlannBorder(PLAN_WINDOW *window)
{
    ...
    if (!window->hideTop)
    {
        if (!window->hideLeft) {
            cur_x++;
            addch(MY_ACS_ULCORNER); //"â”Œ"
        }
        for (; cur_x < window->x + window->w-1; cur_x++) {
            addch(MY_ACS_HLINE); //"â”€"
        }
        addch(MY_ACS_URCORNER); //"â”"
        cur_y++;
        cur_x = window->x;
    }
    ...
    if (!window->hideLeft) {
        cur_x++;
        addch(MY_ACS_LLCORNER); //"â””"
    }
	for (; cur_x < window->x + window->w-1; cur_x++) {
		addch(MY_ACS_HLINE); //"â”€"
	}
	addch(MY_ACS_LRCORNER); //"â”˜"
}
...
#define YUV_TO_RGB(Y, U, V, R, G, B) \
    ...
    struct {
        unsigned int bfSize;      ///< ÓÃ×Ö½Ú±íÊ¾µÄÕû¸öÎÄ¼þ´óÐ¡
        unsigned int bfReserved;  ///< ±£Áô£¬Ò»°ãÉèÖÃÎª0
        unsigned int bfOffBits;   ///< Î»Í¼Êý¾ÝÆ«ÒÆ
        unsigned int biSize;      ///< Î»Í¼ÐÅÏ¢Í·µÄ´óÐ¡£¬Í¨³£ÊÇ28H
        unsigned int biWidth;     ///< Î»Í¼ÏñËØ¿í¶È
        unsigned int biHeight;    ///< Î»Í¼ÏñËØ¸ß¶È
        unsigned short biPlanes;    ///< Î»Í¼µÄÎ»ÃæÊý£¬×ÜÊÇ1
        unsigned short biBitCount;  ///< ÏñËØÎ»¿í
        unsigned int biCompression;   ///< Ñ¹ËõËµÃ÷
        unsigned int biSizeImage;     ///< Î»Í¼Êý¾ÝµÄ´óÐ¡£¬4×Ö½ÚµÄ±¶Êý
        unsigned int biXPelsPerMeter; ///< ÓÃÏóËØ/Ã×±íÊ¾µÄË®Æ½·Ö±æÂÊ£¬Ä¬ÈÏ0x0EC4
        unsigned int biYPelsPerMeter; ///< ÓÃÏóËØ/Ã×±íÊ¾µÄ´¹Ö±·Ö±æÂÊ£¬Ä¬ÈÏ0x0EC4
        unsigned int biClrUsed;       ///< Î»Í¼Ê¹ÓÃµÄÑÕÉ«Êý
